...
public class AllNew {
	...
	public void testTypeCreate() {
		...
		for (int i = 0; i < 100; i++) {
			...
			userProfileService.registerUser(user, userCredentials);
		}
		UserCredentials userAdmin;
		userAdmin = userProfileService.getUserCredentials(1L);
		userAdmin.setStatus(StatusUser.ADMIN);
		userProfileService.updateUserCredentials(userAdmin);
		UserCredentials userCommander;
		userCommander = userProfileService.getUserCredentials(5000L);
		userCommander.setStatus(StatusUser.COMMANDER);
		userProfileService.updateUserCredentials(userCommander);
		...
		type.setParentType(typeService.findType(filter).get(0));
		typeService.registerType(type);
		type.setId(null);
		...
		product.setTypes(typeService.findType(filterType).get(0));
		...
		for (int k = 1; k <= 10; k++) {
			...
			for (int i = 1; i <= 100; i++) {
				...
				pack.setPenalty(penalty);
				pack.setPercentPenalty(percentFine);
				if (i % 10 == 0) {
					n++;
					nn++;
				}
				pack.setRecipient(recipientService.getRecipient((long) n));
